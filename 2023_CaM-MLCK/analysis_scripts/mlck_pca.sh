...
systems=(cam_t34k mcam cam_k30e cam_g40d cam_dN cam_m36i scam1 scam4)
nRep=50
start=143
end=403
...
cat >> tmp.trajin << EOD
rms test ref [crd] :${start}-${end}@CA,C,N,O
...
cat >> tmp.trajin << EOD
readdata $covarFile
rms test ref [crd] :$((start+1))-$((end+1))@CA,C,N,O
...
cat >> tmp.trajin << EOD
readdata $covarFile
rms test ref [crd] :$((start - 4))-$((end - 4))@CA,C,N,O
projection md_project evecs $covarFile out md_project_${sys}_${comp} beg 1 end 3 :$((start - 4))-$((end - 4))@CA,C,N,O
