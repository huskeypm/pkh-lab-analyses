"""Create the input files from templates for run008

Variables that must be set in the mesh template file (.geo):

  - fixed_left = sequence of fixed inclusion tuples for left half of channel
      Each tuple provides information for (x,y,r) for a single fixed inclusion,
        where x and y are normalized by slot_length and d, respectively, and r is not normalized
  - fixed_right = sequence of fixed inclustion tuples for right half of channel 
...
class LeftInclusions(InclusionData):
  def __init__(self,slot_length,d,ncirc,fixed):
    self.box=random_inclusions_2D.Rect(-slot_length/2,-d/2,0,d/2)
    self.fixed=[(slot_length*x,d*y,r) for x,y,r in fixed]
    self.ncirc=ncirc
...
def calc_tmpl_mesh(mesh_values):
  ...
  global inc_id
  inc_id=inc_id_start
  incdata_l=LeftInclusions(mesh_values['slot_length'],mesh_values['d'],mesh_values['n_left'],mesh_values['fixed_left'])
  incdata_r=RightInclusions(mesh_values['slot_length'],mesh_values['d'],mesh_values['n_right'],mesh_values['fixed_right'])
...
def calc_tmpl_model(mesh_data, model_values):
  ...
  for k in ['basename','meshname','modelname',
            'left_conc_A','left_conc_B','left_conc_C',
            'right_conc_A','right_conc_B','right_conc_C',
            'left_potential','right_potential',
            'r_inc_conc_A','r_inc_conc_B','rAB_inc_potential','rBC_inc_potential',
