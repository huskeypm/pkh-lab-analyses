...
#gets stdin selmode
set selmode all
set file sasa_hphil.txt
# selection
set sel [atomselect top "$selmode"]
set hphil [atomselect top "protein and not hydrophobic and noh"]
set n [molinfo top get numframes]
set output [open $file w]
# sasa calculation loop
for {set i 0} {$i < $n} {incr i} {
	molinfo top set frame $i
	set sasa [measure sasa 1.4 $hphil -restrict $sel]
	puts "\t \t progress: $i/$n"
	puts $output "$sasa"
}
puts "\t \t progress: $n/$n"
puts "Done."	
puts "output file: SASA_hphil.dat"
close $output
##}
