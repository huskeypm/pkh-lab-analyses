...
def Cylinder(centers,radius=1e-9,boxSize=15e-9, length = 1e-8, revH = 1e-8,bR=9e-9):
...
"""
    ...
    ########################### TESTING TO ADD AQP
    if 1:
        text+="out%s[]=Extrude{0,0,-aqpH}{Surface{13};};\n"%(cirNum)

    text+= "Plane Surface(1) = {1,16,%s};\n"%(lineNames)
    #text+="NewSurf1[]=Translate{0,0,%s}{ Duplicata{Surface{1};}};"%(length)
    #texts= text[:-2] #this is stupid ugly
    text+= "Plane Surface(2) = {2,%s};\n"%(listExtrudedCircleNames)
    text+= "Surface Loop(1) = {1,%s,6,5,3,7,4,out%s[0],out%s[2],out%s[3],out%s[4],out%s[5]};\n"%(lineNames,cirNum,cirNum,cirNum,cirNum,cirNum)
    text+= "Volume(%s) = {1};\n"%(cirNum)
...
Field[1].VIn = 3e-10;
Field[1].VOut = 1e-9;
Field[1].XMin = -boxSize-radius;
Field[1].XMax = boxSize + radius;
Field[1].YMin = -boxSize-radius;
Field[1].YMax = boxSize+radius;
Field[1].ZMax = length+radius;
Field[1].ZMin = -radius;

Field[2] = Box;
Field[2].VIn = lc/5;
Field[2].VOut = lc;
Field[2].XMin = -boxSize-radius;
Field[2].XMax = boxSize + radius;
Field[2].YMin = -boxSize-radius;
Field[2].YMax = boxSize+radius;
Field[2].ZMax = length+radius;
Field[2].ZMin = -radius;
"""
    ...
    text+="Physical Surface(5) = {out%s[0]};\n"%(cirNum)  ### This is the top of the aquaporin
    text+="Physical Surface(6) = {out%s[2],out%s[3],out%s[4],out%s[5]};\n"%(cirNum,cirNum,cirNum,cirNum)  ### These are the walls of the aquaporin
    text+="Physical Surface(7) = {%s};\n"%(shorterString)   ### These are the pore bots
    text+="Physical Surface(8) = {%s};\n"%(lineNames) ### These are the pore tops
    #counter = 7
    #print "poreTops shape", np.shape(poreTops)
    #for i,item in enumerate(poreTops):
    #        text+= "Physical Surface(%s) = {%s};\n"%(counter,item) ### These are the pore bots
    #        counter+=1
    #        text+="Physical Surface(%s) = {%s};\n"%(counter,poreBot[i])  ### These are the pore walls
    #        counter+=1
    ...
    if num%2!=0:
        while i <num:
            ...
            while j < num:
                ...
                if (abs(x)-radius/2)**2+(abs(y)-radius/2)**2<(bR*1.02)**2:
                    this = False
                else:
                    centers.append((x,y))
                j+=1
...
def Build(num, radius,bR,boxSize, length, revH,name="test"):
    centers = getCenters(num,radius,bR)
    text = Cylinder(centers,radius,boxSize,length,revH,bR)
    print "post cylinder"
    #fileName = "AQ_{:1}_{:1}_{:1}".format(num,bR,boxSize)+".geo"
    fileName = name +".geo"
    fileLines = []
...
Notes:
  ...
  Example: python nanoporousMesher.py -makem "8,1e-9,9e-9,15e-9,10e-9,10e-9,name"
...
if __name__ == "__main__":
  ...
  for i,arg in enumerate(sys.argv):
    ...
    if(arg=="-makem"):
      ...
      print myspl[0]
      print myspl[1]
      Build(num=float(myspl[0]),radius = float(myspl[1]),bR=float(myspl[2]),boxSize=float(myspl[3]),length=float(myspl[4]),revH = float(myspl[5]),name=myspl[6])
      quit()
